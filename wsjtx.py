...
class _WSPacket:
  ...
  @property
  def ClientId(self):
    """WSJT-X network message Id / target unique key."""
...
class WSHaltTx(_WSPacket):
  ...
  The protocol field is named ``Auto Tx Only`` in WSJT-X:
    self.mode = True
        Disable automatic Tx only; do not force an immediate RF abort.
    self.mode = False
        Halt the current transmission immediately.

  ``mode`` is retained as the Python attribute name for compatibility with the
  original FT8Commander code.
  """
  def __init__(self, pkt=None):
    super().__init__(pkt)
...
class WSSwitchConfiguration(_WSPacket):
  ...
  def _encode(self):
    super()._encode()
    self._set_string(self._data['ConfigName'])

  @property
  def ConfigName(self):
    return self._data.get('ConfigName')

  @ConfigName.setter
  def ConfigName(self, val):
    assert isinstance(val, str), 'Configuration name must be a string'
    self._data['ConfigName'] = val
